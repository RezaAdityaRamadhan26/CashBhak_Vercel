...
export async function register(formData) {
  ...
  try {
    ...
    // Insert user dengan status belum terverifikasi
    await connection.execute(
      "INSERT INTO users (username, email, password, is_verified, verification_token) VALUES (?, ?, ?, ?, ?)",
      [username, email, password1, false, verificationToken]
    );
    ...
    return { error: "Sistem Gagal (Detail): " + util.inspect(error).substring(0, 800) };
  }
...
export async function pushProduct(formData) {
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO products (user_id, product_name, barcode, price, stock, product_image)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [userId, product_name, barcode || null, price, stock || 0, product_image]
    );
...
export async function updateProduct(formData) {
  try {
    ...
    const [result] = await connection.execute(
      `UPDATE products 
       SET product_name = ?, barcode = ?, price = ?, stock = ?, product_image = ? 
       WHERE product_id = ?`,
      [product_name, barcode || null, price, stock, product_image, product_id]
    );
